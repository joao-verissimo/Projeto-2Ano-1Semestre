...
sens_humd_solo:
    ...
    movb %dl, %al                   # move the byte in %dl into %al
    sarb $7, %al                    # shift the byte in %al to the right by 7 bits
    movb %al, %r8b                  # move the byte in %al into %r8b
    xorb %dl,%r8b                   # xor the byte in %dl with the byte in %r8b
    subb %al, %r8b                  # subtract the byte in %al from the byte in %r8b


    movb %dil, %al                  # move the byte in %dil into %al
